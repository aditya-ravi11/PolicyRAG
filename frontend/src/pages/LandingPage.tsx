...
const LandingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-surface-50 dark:bg-surface-950 text-surface-900 dark:text-surface-100">
      {/* Nav */}
      <nav className="border-b border-surface-200 dark:border-surface-800 px-6 py-4">
      ...
      <section className="px-6 py-20 md:py-32">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-4xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-brand-400 to-brand-300 bg-clip-text text-transparent">
            SEC Filing Intelligence
            <br />
            You Can Trust
          </h1>
          <p className="text-lg md:text-xl text-surface-500 dark:text-surface-400 mb-10 max-w-2xl mx-auto">
      ...
      {/* Features */}
      <section id="features" className="px-6 py-16 bg-white/50 dark:bg-surface-900/50">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {FEATURES.map((f) => (
              <div key={f.title} className="card p-6">
                <div className="w-12 h-12 bg-brand-600/20 rounded-xl flex items-center justify-center text-brand-600 dark:text-brand-400 mb-4">
                  {f.icon}
                </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {STEPS.map((s) => (
              <div key={s.step} className="text-center">
                <div className="w-12 h-12 bg-brand-600 rounded-full flex items-center justify-center text-white text-lg font-bold mx-auto mb-4">
                  {s.step}
                </div>
      ...
      {/* Demo Questions */}
      <section className="px-6 py-16 bg-white/50 dark:bg-surface-900/50">
          ...
          <div className="space-y-3">
            {DEMO_QUESTIONS.map((q, i) => (
              <button
                ...
                className="w-full text-left px-6 py-4 card hover:border-brand-500/50 transition-all text-surface-600 dark:text-surface-300 hover:text-surface-900 dark:hover:text-surface-100"
              >
                {q}
              </button>
            ))}
      ...
      {/* Footer */}
      <footer className="border-t border-surface-200 dark:border-surface-800 px-6 py-8">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="text-xs text-surface-400 dark:text-surface-600">
